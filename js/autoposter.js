	...
	GrabPressAutoposter = {
		...
		initSearchForm: function() {
			...
			/*if ( 0 === selectedProviders.length ) {
				...
				allProviders.attr( 'selected', 'selected' );
			}

			// If no channels selected
			if ( 0 === selectedChannels.length ) {
				// Select all channels
				allChannels.attr( 'selected', 'selected' );
			}*/

			// Loop through post categories
			// for ( var i = 0; i < allPostCategories.length; i++ ) {
			// 	// If category value is a selected category
			// 	// FIXME: selectedCategories does not exist currently
			// 	if ( -1 < $.inArray( $( allPostCategories[ i ] ).val(), this.selectedCategories ) ) {
			// 		// Add selected class to category
			// 		$( allPostCategories[ i ] ).attr( 'selected', 'selected' );
			// 	}
			// }

			// TODO: Remove when FIXME above is resolved
			// for(var i=0;i<category_options.length; i++){
			// 		if($.inArray($(category_options[i]).val(),GrabPressAutoposter.selectedCategories)>-1){
