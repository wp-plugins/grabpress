...
if ( ! class_exists( 'Grabpress' ) ) {
	...
	class Grabpress {
		...
		static function abort( $message ) {
			...
			//TODO: please root out the reason this is being triggered and fix the code so it doesn't get called, rather than commenting out its effects. silencing errors is not good practice.
		...
		static function enqueue_scripts( $page ) {
			...
			wp_enqueue_script(
				'jquery-copy',
				$plugin_url . 'js/ZeroClipboard.js',
				array( 'jquery' )
			);
			wp_enqueue_script(
				'grab-player',
