...
if ( ! class_exists( 'Grabpress_Views' ) ) {
	...
	class Grabpress_Views {
		...
		/**
		 * [delete-alert_callback description]
		 */
		static function delete_alert_callback() {
			// Reference global WordPress DB
			global $wpdb;

			// Get alert ID from request
			$alert_id = intval( $_REQUEST['alert_id'] );

			// Try calling API for delete alert
			try{
				// Get connector ID from API
				$connector_id = Grabpress_API::get_connector_id();
				// Make DELETE call
				Grabpress_API::call( 'DELETE', '/connectors/' . $connector_id . '/messages/' . $alert_id . '?api_key=' . Grabpress::$api_key, $alert_id );
			} catch ( Exception $e ) { // If DELETE call unsuccessful
				// Log exception error message
				Grabpress::log( 'API call exception: ' . $e->getMessage() );
			}
		...
		/**
		 * [delete_feed_callback description]
		 */
		static function delete_feed_callback() {
			// Reference global WordPress DB
			global $wpdb;

			// Get feed ID from request in the form of an integer
			$feed_id = intval( $_REQUEST['feed_id'] );

			// Try deleting feed from API
			try{
				// Get connector ID
				$connector_id = Grabpress_API::get_connector_id();
				// Make DELETE call
				Grabpress_API::call( 'DELETE', '/connectors/' . $connector_id . '/feeds/' . $feed_id . '?api_key=' . Grabpress::$api_key, $feed_id );
			} catch ( Exception $e ) { // If DELETE unsuccessful
				// Log exception error message
				Grabpress::log( 'API call exception: ' . $e->getMessage() );
			}
		...
		static function edit_feed( $params ) {
			...
			if ( Grabpress_API::validate_key() ) {
				...
				$queried_providers = explode( ',', $query['providers'] );
				$queried_channels  = explode( ',', $query['channels'] );
			...
			if ( isset( $params['email'], $params['password'] ) ) {
				...
				try {
					...
					if ( isset( $user_data->user ) ) {
						...
						// Log connector and user IDs for the PUT
						Grabpress::log( 'Putting to connector ' . Grabpress_API::get_connector_id() . ':' . $user->id );
