				...
				<fieldset id="preview-feed">
					...
					<div class="label-tile">
						...
						<div class="tile-right">
							<select name="channels[]" id="channel-select-preview" class=" multiselect" multiple="multiple" style="width:500px;" onchange="GrabPressCatalog.doValidation(); ">
								<?php
					...
					<div class="label-tile">
						...
						<div class="tile-right">
							<select name="providers[]" id="provider-select-preview" class="multiselect" multiple="multiple" style="<?php esc_attr( Grabpress::outline_invalid() ); ?>" onchange="GrabPressCatalog.doValidation()" >
								<?php
					...
					<div class="label-tile">
						<div class="tile-left">
							<input type="button"  class="button-primary" value="Clear Dates " id="clearDates" style="float:left" >
						</div>
						<div class="tile-right">
							<a href="#" id="clear-search" onclick="return false;" >clear search</a>
							<input type="submit" value="Search " class="update-search button-primary" id="update-search" >
						</div>
					</div>
					<div class="clear"></div>
					<div class="label-tile-one-column">
						...
						if ( isset( $form['display'] ) && 'Tab' == $form['display'] ) {
							if( ! empty( $list_feeds['results'] ) && Grabpress::check_permissions_for( 'gp-video-feeds' ) ) {
								?>
								...
								foreach ( $list_feeds['results'] as $key => $result ) {
									...
									<div data-id="<?php echo $result['video']['video_product_id']; ?>" class="result-tile" <?php if ( 0 == $key && $form['action'] == 'gp_get_preview' ) echo 'style="border-top:none;"'; ?> >
										...
										<div class="tile-right">
											...
											<p class="video_date" style="color:#777">
												...
												<?php if ( 'gp_get_catalog' == $form['action'] ) {
													...
													<input type="button" class="btn-create-feed-single button-primary" value="<?php _e( 'Insert into Post' ) ?>" id="btn-create-feed-single-<?php echo $result['video']['id']; ?>" />
													<input type="button" class="update-search button-primary" onclick="grabModal.play('<?php echo $result['video']['guid']; ?>')" value="Watch Video" /></p>
													<?php }
												} else { ?>
												<input type="button" class="update-search button-primary" 
												onclick="grabModal.play('<?php echo $result['video']['guid']; ?>')" 
												value="Watch Video" /></p>
												<?php } ?> 
											</div>
